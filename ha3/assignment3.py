...
def train_lda(X,Y):
    ...
    # calculate means -- by Max
    mupos = sp.matrix(sp.mean(X[:,Y>0],axis=1)).T
    muneg = sp.matrix(sp.mean(X[:,Y<0],axis=1)).T

    x_pos = sp.matrix(X[:,Y>0])
    x_neg = sp.matrix(X[:,Y<0])

    num_pos = sp.size(x_pos, axis=1)
    num_neg = sp.size(x_neg, axis=1)

    s_B = (mupos-muneg)*(mupos-muneg).T

    sum1 = sp.matrix([[0.0, 0.0], [0.0, 0.0]])
    for i in range(num_pos):
        # print(sum1)
        sum1 = sum1 + ((x_pos[:,i]-mupos)*(x_pos[:,i]-mupos).T)
    sum1 = (1.0/num_pos) * sum1

    sum2 = sp.matrix([[0.0, 0.0], [0.0, 0.0]])
    for i in range(num_pos):
        # print(sum2)
        sum2 = sum2 + ((x_neg[:,i]-muneg)*(x_neg[:,i]-muneg).T)
    sum2 = (1.0/num_neg) * sum2

    s_W = sum1 + sum2

    optimal_w = sp.linalg.inv(s_W)*(mupos-muneg)


    # pdb.set_trace()

    # Does this really work?
    b = ((optimal_w.T).dot(mupos) + (optimal_w.T).dot(muneg))/2.
    # pdb.set_trace()

    return sp.asarray(optimal_w.T)[0], sp.asarray(b)[0][0]
