...
def train_lda(X,Y):
    ...
    mean_pos = sp.array([sp.mean(X[:,Y>0],axis=1)]).T
    mean_neg = sp.array([sp.mean(X[:,Y<0],axis=1)]).T

    x_pos = X[:,Y>0]
    x_neg = X[:,Y<0]

    # The forloop was the same... just slower and ugly
    AAT = lambda A: A.dot(A.T)
    sum1 = AAT(x_pos - mean_pos)/x_pos.shape[1]
    sum2 = AAT(x_neg - mean_neg)/x_neg.shape[1]
